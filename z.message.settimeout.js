...
;Z.$package('Z.message', function(z){
    ...
    var getExecuteElement = function(){
        return executeElement || (executeElement = document.createElement('div'));
    }
    
    var createExecuteId = function(){
        return (+new Date) + '' + (executeId++);
    }
    
    var wrapFunc = function(func, params){
        return function(){
    ...
    var notify = function(type, message){
        ...
        for(var i in listenerList[type]){
            var func = listenerList[type][i];
            
            setTimeout(wrapFunc(func, [type, message]), 0);
        }
